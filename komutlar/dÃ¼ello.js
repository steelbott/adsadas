...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
	...
	if (!opponent) return message.reply("**Oynamak istediğin kişiyi etiketlemelisin** <:arama:852966591213338666>")
  ...
  if (opponent.id === message.author.id) return message.reply('Kendin ile düello Atamazsın');
		if (this.fighting.has(message.channel.id)) return message.reply('<a:uyari:846435581806641212> | **Kanal başına sadece bir düello meydana gelebilir.**');
		this.fighting.add(message.channel.id);
		try {
			if (!opponent.bot) {
                await message.channel.send(`<:duello:854730145021689936> ${opponent}, **düello isteği geldi.** **Düello'yu kabul ediyor musun?** (\`evet\` veya \`hayir\` **olarak cevap veriniz.**) <a:ates:854021958094422026>`);
				const verification = await verify(message.channel, opponent);
				if (!verification) {
					this.fighting.delete(message.channel.id);
					return message.channel.send(`<a:hata:846429181138305025> Düello kabul edilmedi...`);
				}
			}
			let userHP = 500;
			let oppoHP = 500;
			...
			return message.channel.send(`Oyun bitti! Ve Son Kapanışı, **${winner}** kazandı! <a:BlueHype:843472272400711751>  \n**${message.author.username}**: ${userHP} :heartpulse: \n**${opponent.username}**: ${oppoHP} :heartpulse:`);
		} catch (err) {
			this.fighting.delete(message.channel.id);
			throw err;
		}
